...
```
- onClick: (event: MouseEvent) => void -  метод, который будет вызван при клике на элементе. Он принимает один параметр event типа MouseEvent, который представляет собой событие клика.
- price: price - свойство, которое представляет собой цену товара
- title: string - свойство, которое представляет собой название товара
- index?: number - войство, которое представляет собой индекс элемента в списке

```

### Интерфейс ISucces
```
-	total: number - говорит что в объекте финального заказала должно быть поле total с финальной ценой
```
...
```
	...
	• getField() - возвращает поле с оплатой
...
``` 
## Класс Modal
Класс для управления модальным окном. Наследуется от Component с типом данных HTMLElement (обязательно должен содержать content).
### Поля класса
```
